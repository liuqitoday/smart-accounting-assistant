...
public record AverageByPeriodResult(String title, AnalysisMetric metric, AnalysisUnit unit,
        TransactionType transactionType, AnalysisResultPeriod period, AnalysisFilters filters,
        List<AnalysisWarning> warnings, List<AnalysisTrendFact.Point> points,
        Values values) implements AnalysisResult {
    public AverageByPeriodResult {
        warnings = warnings == null ? List.of() : List.copyOf(warnings);
        points = points == null ? List.of() : List.copyOf(points);
    }

    public record Values(BigDecimal average) {}

    @Override
    @JsonProperty("kind")
    public AnalysisResultKind kind() { return AnalysisResultKind.AVERAGE_BY_PERIOD; }
}
